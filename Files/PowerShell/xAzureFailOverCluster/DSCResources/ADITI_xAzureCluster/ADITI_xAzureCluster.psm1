    ...
    try
    {
        ...
        {            
			...
			if (!$clusterNameRes -OR $clusterNameRes.State -ne "Online")
			{
				...
				$clusterSize = $NumberOfClusterNodes + 1
			...
			#
			# Now, add the secondary cluster nodes
			#
			$allNodes = Get-WmiObject -namespace "root\mscluster" -class MSCluster_Node
			for($i = 0; $i -lt ($NumberOfClusterNodes - 1); $i++)
			{
				$nodeName = $ClusterNodeNamePrefix + ($i + 2)
        ...
        else
        {
            try
            {
                ...
                if ($cluster -ne $null -and $cluster.Name -eq $Name)
                {
					...
					$bRet = $nodeCount -eq $NumberOfClusterNodes
                    if ($bRet)
                    {
